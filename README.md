...
## MTU: tunnel MTU и path MTU

Скрипт различает две величины.

### Outer/path MTU

Это MTU внешнего пути, по которому реально идут UDP-пакеты AWG. Он используется для оценки безопасного размера junk-пакетов и консервативной границы tunnel MTU.
...
Поэтому `Jmax` ограничивается внешним/path MTU, а не просто значением `MTU = ...` внутри `awg0.conf`.

### AWG interface MTU

Это MTU TUN-интерфейса AWG. Он участвует в генерации CPS-пакетов `I1-I5` и в расчётах transport padding.

Скрипт показывает консервативную границу без внешней фрагментации с учётом `S4` и transport overhead и предупреждает, если выбранный tunnel MTU её превышает.
