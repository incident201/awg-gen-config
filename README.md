...
- спрашивает одно значение `MTU` для генерации CPS-пакетов;
...
В интерактивном режиме используется одно поле `MTU`, соответствующее полю MTU в Web Architect:

```text
MTU presets: 1500 Ethernet, 1420 WG / PPPoE, 1280 min IPv6
MTU [1500]:
```

Допустимый диапазон — `576–9000`. Выбранное значение передаётся как `CpsOptions.mtu` и используется при построении CPS-пакетов `I1-I5`, включая расчёт их padding. Оно не влияет на генерацию `Jc`, `Jmin`, `Jmax` и параметров `S1-S4`.

MTU — это вход генератора, соответствующий полю MTU в Web Architect. Он не записывается в серверный `awg0.conf` и не изменяет существующий MTU серверного интерфейса.
...
- соответствие CPS padding выбранному MTU;
- независимость диапазонов `Jmin/Jmax` от MTU;
- отсутствие лишних сетевых вопросов в интерактивном интерфейсе;
